...
def test_associate():
    ...
    with vcr.use_cassette(tape_file):
        one_to_one = pd.DataFrame({'time': [datetime(2019, 7, 6, 3, 19, 53)],
                                   'latitude': [35.770],
                                   'longitude': [-117.599],
                                   'magnitude': 7.1})
        ...
        two_events = pd.DataFrame({'time': [datetime(2019, 7, 6, 9, 30, 15),
        ...
        datafile = pathlib.Path(__file__).parent / '..' / \
            'data' / 'sample_catalogue.csv'
        ...
        dataframe['time'] = dataframe['time'] = pd.to_datetime(
            dataframe[tcols])
        dataframe = dataframe.sort_values('time')

        seconds_frame = dataframe[dataframe['accuracy'] == 0]
        associated_seconds, alternate_seconds = associate(seconds_frame,
                                                          time_tol_secs=60,
                                                          mag_column='MAGNITUDE',
                                                          lat_column='LATITUDE',
                                                          lon_column='LONGITUDE'
                                                          )
        cmplist = ['ushis474', 'ushis524', 'ushis529',
                   'ushis532', 'ushis533', 'ushis541']
        assert sorted(associated_seconds['comcat_id'].tolist()) == cmplist

        minutes_frame = dataframe[dataframe['accuracy'] == 60]
        associated_minutes, alternate_minutes = associate(minutes_frame,
                                                          time_tol_secs=300,
                                                          mag_column='MAGNITUDE',
                                                          lat_column='LATITUDE',
                                                          lon_column='LONGITUDE'
        ...
        associated_hours, alternate_hours = associate(hours_frame,
                                                      time_tol_secs=3600,
                                                      mag_column='MAGNITUDE',
                                                      lat_column='LATITUDE',
                                                      lon_column='LONGITUDE'
        ...
        associated_days, alternate_days = associate(days_frame,
                                                    time_tol_secs=86400,
                                                    mag_column='MAGNITUDE',
                                                    lat_column='LATITUDE',
                                                    lon_column='LONGITUDE'
        ...
        missing_loc = pd.DataFrame({'time': [datetime(1905, 4, 4, 0, 49, 59)],
                                    'latitude': [np.nan],
                                    'longitude': [np.nan],
                                    'magnitude': [7.9]})
        ...
        missing_both = pd.DataFrame({'time': [datetime(1905, 4, 4, 0, 49, 59)],
                                     'latitude': [np.nan],
                                     'longitude': [np.nan],
                                     'magnitude': [np.nan]})
