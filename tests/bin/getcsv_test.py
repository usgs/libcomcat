...
def test_getcsv():
    ...
    try:
        cmd = ('getcsv %s -x -r -1.020000 -19.128000 1 -s '
               '1900-01-03T00:02:18 -e 1900-01-03T00:05:18' %
               temp_file)
    ...
    try:
        cmd = ('getcsv %s -x -r -1.020000 -19.128000 1 -s '
               '1918-06-03T00:02:18 -e 1918-06-03T00:05:18' %
               temp_file)
        res, stdout, stderr = get_command_output(cmd)
        if not res:
            raise AssertionError(
    ...
    try:
        cmd = ('getcsv %s -x -r -1.020000 -19.128000 1 -s '
               '1900-06-03T00:02:18 -e 1900-06-03T00:05:18' %
               temp_file)
        res, stdout, stderr = get_command_output(cmd)
        if not res:
            raise AssertionError(
    ...
    temp_file = os.path.join(tmpdir, 'temp.csv')
    try:
        cmd = ('getcsv %s -x -b 163.213 -178.945 -48.980 -32.324'
    ...
    temp_file = os.path.join(tmpdir, 'temp.csv')
    try:
        cmd = ('getcsv %s -r -1.020000 -19.128000 1 -s '
               '1918-06-03T00:02:18 -e 1918-06-03T00:05:18 --country abs' %
               temp_file)
        res, stdout, stderr = get_command_output(cmd)
    ...
    temp_file = os.path.join(tmpdir, 'temp.csv')
    try:
        cmd = ('getcsv %s --country abs' %
               temp_file)
        res, stdout, stderr = get_command_output(cmd)
        assert b'abs is not a valid ISO 3166 country code. See https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2 for the list.' in stderr
    except Exception as e:
        raise(e)
    finally:
        shutil.rmtree(tmpdir)
    ...
    try:
        cmd = ('getcsv %s -b 163.213 -178.945 -48.980 -32.324'
               ' -s 2013-01-01 -e 2014-01-01 -f tab' %
               temp_file)
        res, stdout, stderr = get_command_output(cmd)
        if not res:
            raise AssertionError(
    ...
    try:
        cmd = ('getcsv %s -b 163.213 -178.945 -48.980 -32.324'
               ' -s 2013-01-01 -e 2014-01-01 -f excel' %
               temp_file)
        res, stdout, stderr = get_command_output(cmd)
        if not res:
            raise AssertionError(
