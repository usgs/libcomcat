...
def _get_product_from_detail(
    detail, tproduct, contents, folder, version, source, list_only=False
):
    if not detail.hasProduct(tproduct):
        return False

    try:
        products = detail.getProducts(tproduct, source=source, version=version)
    except ProductNotFoundError:
        print(
            "No %s product found for event %s and source %s. Skipping."
...
def get_parser():
    ...
    parser.add_argument(
        "--get-version",
        dest="version",
        choices=["first", "last", "all", "preferred"],
    ...
    parser.add_argument(
        ...
        type=float,
        nargs=3,
        help="Search radius in kilometers (radius"
...
def main():
    ...
    if args.eventid:
        detail = get_event_by_id(
            args.eventid, includesuperseded=get_superseded, scenario=args.scenario
        )
        _get_product_from_detail(
            detail,
            args.product,
            args.contents,
            args.outputFolder,
    ...
    if args.startTime is None:
        ...
        print(
            "You did not specify a search start time, defaulting to %s" % str(starttime)
        )
    else:
    ...
    if args.endTime is None:
        ...
        print("You did not specify a search end time, defaulting to %s" % str(endtime))
    else:
    ...
    for event in events:
        ...
        try:
            detail = event.getDetailEvent(
                includesuperseded=get_superseded, scenario=args.scenario
            )
        except Exception:
            print(
